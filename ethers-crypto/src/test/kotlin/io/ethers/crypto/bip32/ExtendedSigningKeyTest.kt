...
class ExtendedSigningKeyTest : FunSpec({
    context("vector tests") {
        TEST_VECTOR_1.forEach { vector ->
            test("test vector 1: ${vector.path}") {
        ...
        TEST_VECTOR_2.forEach { vector ->
            test("test vector 2: ${vector.path}") {
                val seed = vector.seedHex.hexToByteArray()
                val key = ExtendedSigningKey.fromSeed(seed).derivePath(HDPath.parse(vector.path))
        ...
        TEST_VECTOR_3_RETENTION_OF_LEADING_ZEROS.forEach { vector ->
            test("test vector 3: ${vector.path}") {
                val seed = vector.seedHex.hexToByteArray()
                val key = ExtendedSigningKey.fromSeed(seed).derivePath(HDPath.parse(vector.path))
        ...
        TEST_VECTOR_4_RETENTION_OF_LEADING_ZEROS.forEach { vector ->
            test("test vector 4: ${vector.path}") {
                val seed = vector.seedHex.hexToByteArray()
                val key = ExtendedSigningKey.fromSeed(seed).derivePath(HDPath.parse(vector.path))
    ...
    private data class TestVector(
        val seedHex: String,
        val path: String,
        private val publicKeyBase58: String,
        private val privateKeyBase58: String,
    ) {
        ...
        init {
            ...
            if (!chainCodePublic.contentEquals(chainCodePrivate)) {
                throw IllegalArgumentException("Public and private chain codes do not match. Check test vectors.")
            }

            this.chainCode = chainCodePublic
        }
    }
    ...
    private companion object {
        ...
        val TEST_VECTOR_3_RETENTION_OF_LEADING_ZEROS = listOf(
            TestVector(
        ...
        val TEST_VECTOR_4_RETENTION_OF_LEADING_ZEROS = listOf(
            TestVector(
