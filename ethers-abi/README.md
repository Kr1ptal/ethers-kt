...
- `AbiCodec`: implements the encoding and decoding logic between encoded ABI and Kotlin types. It also provides support for
...
- `ContractStruct`, `ContractEvent`, `CustomContractError`: represent the actual Kotlin types for structs, events, and errors. They
...
data class Approval(
    ...
    companion object : EventFactory<Approval> {
        ...
        override fun filter(provider: Middleware): EventFilterBase<Approval, *> {
            return EventFilter(provider, this)
        }

        override fun decode(log: Log, `data`: List<Any>): Approval {
            return Approval(
                data[0] as Address,
                data[1] as Address,
